...
exports.generateEphmeralKeyPair = function (curve, callback) {
  nodeify(crypto.subtle.generateKey({
    ...
    var genSharedKey = function genSharedKey(theirPub, forcePrivate, cb) {
      ...
      if (forcePrivate) {
        privateKey = crypto.subtle.importKey('jwk', unmarshalPrivateKey(curve, forcePrivate), {
          name: 'ECDH',
          namedCurve: curve
        }, false, ['deriveBits']);
      } else {
      ...
      var keys = Promise.all([crypto.subtle.importKey('jwk', unmarshalPublicKey(curve, theirPub), {
        name: 'ECDH',
        namedCurve: curve
      }, false, []), privateKey]);
    ...
    return crypto.subtle.exportKey('jwk', pair.publicKey).then(function (publicKey) {
      return {
